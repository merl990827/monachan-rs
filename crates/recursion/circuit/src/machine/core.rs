...
use monerochan_recursion_core::air::PV_DIGEST_NUM_WORDS;
...
use monerochan_stark::{ShardProof, StarkGenericConfig, StarkVerifyingKey};

use monerochan_recursion_compiler::{
...
pub struct MONEROCHANRecursionWitnessVariable<
...
pub struct MONEROCHANRecursionWitnessValues<SC: StarkGenericConfig> {
...
pub struct MONEROCHANRecursionShape {
    pub proof_shapes: Vec<OrderedShape>,
    pub is_complete: bool,
}

/// A program for recursively verifying a batch of MONEROCHAN proofs.
#[derive(Debug, Clone, Copy)]
pub struct MONEROCHANRecursiveVerifier<C: Config, SC: BabyBearFriConfig> {
    _phantom: PhantomData<(C, SC)>,
}

impl<C, SC> MONEROCHANRecursiveVerifier<C, SC>
where
...
{
    /// Verify a batch of MONEROCHAN shard proofs and aggregate their public values.
    ///
    /// This program represents a first recursive step in the verification of an MONEROCHAN proof
    ...
    /// See [MONEROCHANProver::verify] for the verification algorithm of a complete MONEROCHAN proof. In this
    ...
    /// [MONEROCHANProver::verify] is that the recursive verifier has no way of reconstructing the
    ...
    pub fn verify(
        ...
        input: MONEROCHANRecursionWitnessVariable<C, SC>,
    ) {
        // Read input.
        let MONEROCHANRecursionWitnessVariable {
            vk,
            shard_proofs,
            is_complete,
            is_first_shard,
        ...
        // Write all values to the public values struct and commit to them.
        {
            ...
            recursion_public_values.monerochan_vk_digest = vk_digest;
...
impl<SC: BabyBearFriConfig> MONEROCHANRecursionWitnessValues<SC> {
    pub fn shape(&self) -> MONEROCHANRecursionShape {
        let proof_shapes = self.shard_proofs.iter().map(|proof| proof.shape()).collect();

        MONEROCHANRecursionShape { proof_shapes, is_complete: self.is_complete }
    }
}

impl MONEROCHANRecursionWitnessValues<BabyBearPoseidon2> {
    pub fn dummy(
        machine: &StarkMachine<BabyBearPoseidon2, RiscvAir<BabyBear>>,
        shape: &MONEROCHANRecursionShape,
    ) -> Self {
...
impl From<OrderedShape> for MONEROCHANRecursionShape {
