...
use monerochan_recursion_compiler::ir::Builder;
...
use monerochan_recursion_compiler::ir::Felt;
...
use super::{
    MONEROCHANCompressWitnessValues, MONEROCHANCompressWitnessVariable, MONEROCHANDeferredWitnessValues,
    MONEROCHANDeferredWitnessVariable, MONEROCHANMerkleProofWitnessValues, MONEROCHANMerkleProofWitnessVariable,
    MONEROCHANRecursionWitnessValues, MONEROCHANRecursionWitnessVariable,
};
...
impl<C> Witnessable<C> for MONEROCHANRecursionWitnessValues<BabyBearPoseidon2>
where
    C: CircuitConfig<F = InnerVal, EF = InnerChallenge, Bit = Felt<InnerVal>>,
{
    type WitnessVariable = MONEROCHANRecursionWitnessVariable<C, BabyBearPoseidon2>;
    ...
    fn read(&self, builder: &mut Builder<C>) -> Self::WitnessVariable {
        ...
        MONEROCHANRecursionWitnessVariable {
            vk,
            shard_proofs,
            is_complete,
            is_first_shard,
...
impl<C: CircuitConfig<F = InnerVal, EF = InnerChallenge>, SC: BabyBearFriConfigVariable<C>>
    Witnessable<C> for MONEROCHANCompressWitnessValues<SC>
where
    Com<SC>: Witnessable<C, WitnessVariable = <SC as FieldHasherVariable<C>>::DigestVariable>,
    OpeningProof<SC>: Witnessable<C, WitnessVariable = TwoAdicPcsProofVariable<C, SC>>,
{
    type WitnessVariable = MONEROCHANCompressWitnessVariable<C, SC>;

    fn read(&self, builder: &mut Builder<C>) -> Self::WitnessVariable {
        let vks_and_proofs = self.vks_and_proofs.read(builder);
        let is_complete = InnerVal::from_bool(self.is_complete).read(builder);

        MONEROCHANCompressWitnessVariable { vks_and_proofs, is_complete }
    }
...
impl<C> Witnessable<C> for MONEROCHANDeferredWitnessValues<BabyBearPoseidon2>
where
    C: CircuitConfig<F = InnerVal, EF = InnerChallenge, Bit = Felt<InnerVal>>,
{
    type WitnessVariable = MONEROCHANDeferredWitnessVariable<C, BabyBearPoseidon2>;
    ...
    fn read(&self, builder: &mut Builder<C>) -> Self::WitnessVariable {
        ...
        let monerochan_vk_digest = self.monerochan_vk_digest.read(builder);
        ...
        MONEROCHANDeferredWitnessVariable {
            vks_and_proofs,
            vk_merkle_data,
    ...
    fn write(&self, witness: &mut impl WitnessWriter<C>) {
        ...
        self.monerochan_vk_digest.write(witness);
...
impl<C: CircuitConfig<F = BabyBear>, SC: BabyBearFriConfigVariable<C>> Witnessable<C>
    for MONEROCHANMerkleProofWitnessValues<SC>
where
...
{
    type WitnessVariable = MONEROCHANMerkleProofWitnessVariable<C, SC>;

    fn read(&self, builder: &mut Builder<C>) -> Self::WitnessVariable {
        MONEROCHANMerkleProofWitnessVariable {
