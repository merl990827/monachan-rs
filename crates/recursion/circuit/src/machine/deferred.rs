...
use monerochan_primitives::consts::WORD_SIZE;
...
use super::{
    assert_complete, recursion_public_values_digest, MONEROCHANCompressShape, MONEROCHANCompressWitnessValues,
    MONEROCHANMerkleProofVerifier, MONEROCHANMerkleProofWitnessValues, MONEROCHANMerkleProofWitnessVariable,
};

pub struct MONEROCHANDeferredVerifier<C, SC, A> {
    _phantom: std::marker::PhantomData<(C, SC, A)>,
}

#[derive(Debug, Clone, Hash)]
pub struct MONEROCHANDeferredShape {
    inner: MONEROCHANCompressShape,
    height: usize,
}
...
pub struct MONEROCHANDeferredWitnessValues<SC: BabyBearFriConfig + FieldHasher<BabyBear>> {
    pub vks_and_proofs: Vec<(StarkVerifyingKey<SC>, ShardProof<SC>)>,
    pub vk_merkle_data: MONEROCHANMerkleProofWitnessValues<SC>,
    pub start_reconstruct_deferred_digest: [SC::Val; POSEIDON_NUM_WORDS],
    pub monerochan_vk_digest: [SC::Val; DIGEST_SIZE],
...
pub struct MONEROCHANDeferredWitnessVariable<
...
> {
    ...
    pub vk_merkle_data: MONEROCHANMerkleProofWitnessVariable<C, SC>,
    pub start_reconstruct_deferred_digest: [Felt<C::F>; POSEIDON_NUM_WORDS],
    pub monerochan_vk_digest: [Felt<C::F>; DIGEST_SIZE],
...
impl<C, SC, A> MONEROCHANDeferredVerifier<C, SC, A>
where
    ...
    A: MachineAir<SC::Val> + for<'a> Air<RecursiveVerifierConstraintFolder<'a, C>>,
{
    ...
    pub fn verify(
        ...
        input: MONEROCHANDeferredWitnessVariable<C, SC>,
        value_assertions: bool,
    ) {
        let MONEROCHANDeferredWitnessVariable {
            vks_and_proofs,
            vk_merkle_data,
            start_reconstruct_deferred_digest,
            monerochan_vk_digest,
        ...
        MONEROCHANMerkleProofVerifier::verify(builder, values, vk_merkle_data, value_assertions);
        ...
        for (vk, shard_proof) in vks_and_proofs {
            ...
            // poseidon2( current_digest[..8] || pv.monerochan_vk_digest[..8] ||
            ...
            inputs[DIGEST_SIZE..DIGEST_SIZE + DIGEST_SIZE]
                .copy_from_slice(&current_public_values.monerochan_vk_digest);
        ...
        // Set the monerochan_vk_digest to be the hitned value.
        deferred_public_values.monerochan_vk_digest = monerochan_vk_digest;
...
impl MONEROCHANDeferredWitnessValues<BabyBearPoseidon2> {
    pub fn dummy<A: MachineAir<BabyBear>>(
        machine: &StarkMachine<BabyBearPoseidon2, A>,
        shape: &MONEROCHANDeferredShape,
    ) -> Self {
        let inner_witness =
            MONEROCHANCompressWitnessValues::<BabyBearPoseidon2>::dummy(machine, &shape.inner);
        let vks_and_proofs = inner_witness.vks_and_proofs;

        let vk_merkle_data = MONEROCHANMerkleProofWitnessValues::dummy(vks_and_proofs.len(), shape.height);

        Self {
            vks_and_proofs,
            vk_merkle_data,
            is_complete: true,
            monerochan_vk_digest: [BabyBear::zero(); DIGEST_SIZE],
...
impl MONEROCHANDeferredShape {
    pub const fn new(inner: MONEROCHANCompressShape, height: usize) -> Self {
        Self { inner, height }
    }
}
