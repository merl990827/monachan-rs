...
use monerochan_recursion_compiler::ir::{Builder, Felt, SymbolicFelt};

use monerochan_recursion_core::air::{RecursionPublicValues, RECURSIVE_PROOF_NUM_PV_ELTS};
...
use monerochan_recursion_compiler::circuit::CircuitV2Builder;
/// A program to verify a batch of recursive proofs and aggregate their public values.
#[derive(Debug, Clone, Copy)]
pub struct MONEROCHANCompressVerifier<C, SC, A> {
...
pub struct MONEROCHANCompressWitnessVariable<
...
pub struct MONEROCHANCompressWitnessValues<SC: StarkGenericConfig> {
    pub vks_and_proofs: Vec<(StarkVerifyingKey<SC>, ShardProof<SC>)>,
    pub is_complete: bool,
}

#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct MONEROCHANCompressShape {
    proof_shapes: Vec<OrderedShape>,
}

impl<C, SC, A> MONEROCHANCompressVerifier<C, SC, A>
where
    SC: BabyBearFriConfigVariable<C>,
    ...
    A: MachineAir<SC::Val> + for<'a> Air<RecursiveVerifierConstraintFolder<'a, C>>,
{
    ...
    /// - Core proofs: proofs which are recursive proof of a batch of MONEROCHAN shard proofs. The
    ...
    ///   checked against itself as in [monerochan_prover::Prover] or as in [super::MONEROCHANRootVerifier].
    pub fn verify(
        ...
        // Read input.
        let MONEROCHANCompressWitnessVariable { vks_and_proofs, is_complete } = input;
        ...
        let mut monerochan_vk_digest: [Felt<_>; DIGEST_SIZE] =
        ...
        for (i, (vk, shard_proof)) in vks_and_proofs.into_iter().enumerate() {
            ...
            if i == 0 {
                ...
                // Initialize the monerochan_vk digest
                for (digest, first_digest) in
                    monerochan_vk_digest.iter_mut().zip(current_public_values.monerochan_vk_digest)
                {
                    *digest = first_digest;
                }
            ...
            // Assert that the monerochan_vk digest is always the same.
            for (digest, current) in monerochan_vk_digest.iter().zip(current_public_values.monerochan_vk_digest) {
        ...
        // Set monerochan_vk digest to the one from the proof values.
        compress_public_values.monerochan_vk_digest = monerochan_vk_digest;
...
impl<SC: BabyBearFriConfig> MONEROCHANCompressWitnessValues<SC> {
    pub fn shape(&self) -> MONEROCHANCompressShape {
        let proof_shapes = self.vks_and_proofs.iter().map(|(_, proof)| proof.shape()).collect();
        MONEROCHANCompressShape { proof_shapes }
    }
}

impl MONEROCHANCompressWitnessValues<BabyBearPoseidon2> {
    pub fn dummy<A: MachineAir<BabyBear>>(
        machine: &StarkMachine<BabyBearPoseidon2, A>,
        shape: &MONEROCHANCompressShape,
    ) -> Self {
...
impl From<Vec<OrderedShape>> for MONEROCHANCompressShape {
