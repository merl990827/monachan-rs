...
use monerochan_core_machine::utils::next_power_of_two;
use monerochan_derive::AlignedBorrow;
use monerochan_stark::air::{BaseAirBuilder, BinomialExtension, ExtensionAirBuilder, MachineAir};

use crate::{air::Block, builder::MONEROCHANRecursionAirBuilder, ExecutionRecord};
...
use {
    ...
    monerochan_core_machine::utils::pad_rows_fixed,
...
impl<F: PrimeField32, const DEGREE: usize> MachineAir<F> for FriFoldChip<DEGREE> {
    ...
    fn generate_preprocessed_trace(&self, _program: &Self::Program) -> Option<RowMajorMatrix<F>> {
        unimplemented!("To generate traces, enable feature `monerochan-recursion-core/sys`");
    }
...
impl<const DEGREE: usize> FriFoldChip<DEGREE> {
    pub fn eval_fri_fold<AB: MONEROCHANRecursionAirBuilder>(
        &self,
        builder: &mut AB,
...
where
    AB: MONEROCHANRecursionAirBuilder + PairBuilder,
{
...
mod tests {
    ...
    use monerochan_core_machine::utils::setup_logger;
    use monerochan_stark::{air::MachineAir, StarkGenericConfig};
    use std::mem::size_of;
