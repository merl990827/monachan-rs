...
// Re-export the `MONEROCHANReduceProof` struct from monerochan_core_machine.
//
// This is done to avoid a circular dependency between monerochan_core_machine and monerochan_core_executor, and
// enable crates that depend on monerochan_core_machine to import the `MONEROCHANReduceProof` type directly.
pub mod reduce {
    pub use monerochan_stark::MONEROCHANReduceProof;
}
...
pub mod programs {
    ...
    #[allow(missing_docs)]
    pub mod tests {
        use monerochan_core_executor::{Instruction, Opcode, Program};
