...
use monerochan_core_executor::{ExecutionRecord, Executor, Program, MONEROCHANContext};
...
use monerochan_stark::{
    ...
    MachineVerificationError, OpeningProof, PcsProverData, ProverConstraintFolder, MONEROCHANCoreOpts,
    StarkGenericConfig, StarkMachine, StarkProvingKey, StarkVerifyingKey, Val,
    VerifierConstraintFolder,
};

use crate::{io::MONEROCHANStdin, riscv::RiscvAir, shape::CoreShapeConfig};
...
/// The canonical entry point for testing a [`Program`] and [`MONEROCHANStdin`] with a [`MachineProver`].
pub fn run_test<P: MachineProver<BabyBearPoseidon2, RiscvAir<BabyBear>>>(
    mut program: Program,
    inputs: MONEROCHANStdin,
) -> Result<MONEROCHANPublicValues, MachineVerificationError<BabyBearPoseidon2>> {
    let shape_config = CoreShapeConfig::<BabyBear>::default();
    shape_config.fix_preprocessed_shape(&mut program).unwrap();

    let runtime = tracing::debug_span!("runtime.run(...)").in_scope(|| {
        let mut runtime = Executor::new(program, MONEROCHANCoreOpts::default());
        runtime.maximal_shapes = Some(
            shape_config
                .maximal_core_shapes(MONEROCHANCoreOpts::default().shard_size.ilog2() as usize)
                .into_iter()
                .collect(),
        );
    ...
    let public_values = MONEROCHANPublicValues::from(&runtime.state.public_values_stream);
...
pub fn run_test_core<P: MachineProver<BabyBearPoseidon2, RiscvAir<BabyBear>>>(
    ...
    let (proof, output, _) = prove_core(
        ...
        &inputs,
        MONEROCHANCoreOpts::default(),
        MONEROCHANContext::default(),
        shape_config,
...
where
    ...
    A::Record: MachineRecord<Config = MONEROCHANCoreOpts>,
    ...
    OpeningProof<SC>: Send + Sync,
{
    ...
    let proof = prover.prove(&pk, records, &mut challenger, MONEROCHANCoreOpts::default()).unwrap();
    prove_span.exit();
