...
use monerochan_stark::{MachineProvingKey, StarkVerifyingKey};
use thiserror::Error;

use p3_field::PrimeField32;
use monerochan_stark::air::MachineAir;
...
use monerochan_core_executor::{
    ...
    Program, MONEROCHANContext,
};
use monerochan_stark::{
    air::PublicValues, shape::OrderedShape, Com, MachineProof, MachineProver, MachineRecord,
    OpeningProof, PcsProverData, MONEROCHANCoreOpts, ShardProof, StarkGenericConfig, Val,
};
    ...
    PcsProverData<SC>: Send + Sync,
{
    ...
    std::thread::scope(move |s| {
        ...
        let checkpoint_generator_handle: ScopedJoinHandle<Result<_, MONEROCHANCoreProverError>> =
            s.spawn(move || {
                ...
                tracing::debug_span!("checkpoint generator").in_scope(|| {
                    ...
                    loop {
                        ...
                        let (checkpoint, _, done) = runtime
                            .execute_state(false)
                            .map_err(MONEROCHANCoreProverError::ExecutionError)?;

                        // Save the checkpoint to a temp file.
                        let mut checkpoint_file =
                            tempfile::tempfile().map_err(MONEROCHANCoreProverError::IoError)?;
                        checkpoint
                            .save(&mut checkpoint_file)
                            .map_err(MONEROCHANCoreProverError::IoError)?;
...
#[derive(Error, Debug)]
pub enum MONEROCHANCoreProverError {
