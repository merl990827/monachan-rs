...
use monerochan_core_executor::syscalls::SyscallCode;
use monerochan_stark::air::{InteractionScope, MONEROCHANAirBuilder, SubAirBuilder};
...
mod test {
    ...
    use monerochan_core_executor::{Program, MONEROCHANContext};
    use monerochan_stark::{
        baby_bear_poseidon2::BabyBearPoseidon2, CpuProver, MachineProver, MONEROCHANCoreOpts,
        StarkGenericConfig,
    };
    ...
    fn test_keccak_random() {
        ...
        .unwrap();
        let mut public_values = MONEROCHANPublicValues::from(&public_values);
