...
use monerochan_stark::{air::MONEROCHANAirBuilder, Word};

use crate::{
    air::{MONEROCHANCoreAirBuilder, WordAirBuilder},
    memory::MemoryCols,
    operations::{BabyBearWordRangeChecker, IsZeroOperation},
};
use monerochan_core_executor::{
...
impl MemoryInstructionsChip {
    ...
    pub(crate) fn compute_opcode<AB: MONEROCHANAirBuilder>(
        &self,
    ...
    ///    address.
    pub(crate) fn eval_memory_address_and_access<AB: MONEROCHANCoreAirBuilder>(
        &self,
        builder: &mut AB,
        local: &MemoryInstructionsColumns<AB::Var>,
        is_real: AB::Expr,
    ...
    pub(crate) fn eval_memory_load<AB: MONEROCHANAirBuilder>(
        &self,
        builder: &mut AB,
        local: &MemoryInstructionsColumns<AB::Var>,
    ) {
        ...
        // SAFETY: If the opcode is not `lb` or `lh`, then `is_lb + is_lh = 0`, so
        // `mem_value_is_neg_not_x0 = 0`. In the other case, `is_lb + is_lh = 1` (at most
        // one selector is on), so `most_sig_byte` and `most_sig_bit` are correct.
        // Since `op_a_0` is known to be correct, we can conclude that `mem_value_is_neg_not_x0` is
        // correct for all cases, including padding rows.
        builder.assert_eq(
        ...
        // SAFETY: As we mentioned before, `mem_value_is_neg_not_x0` is correct in all cases and
    ...
    pub(crate) fn eval_memory_store<AB: MONEROCHANAirBuilder>(
        &self,
        builder: &mut AB,
        local: &MemoryInstructionsColumns<AB::Var>,
    ) {
    ...
    /// instructions.
    pub(crate) fn eval_unsigned_mem_value<AB: MONEROCHANAirBuilder>(
        &self,
        builder: &mut AB,
        local: &MemoryInstructionsColumns<AB::Var>,
    ) {
    ...
    pub(crate) fn eval_offset_value_flags<AB: MONEROCHANAirBuilder>(
        &self,
        builder: &mut AB,
        local: &MemoryInstructionsColumns<AB::Var>,
    ) {
