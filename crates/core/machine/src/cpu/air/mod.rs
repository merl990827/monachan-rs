...
use monerochan_core_executor::{ByteOpcode, DEFAULT_PC_INC};
use monerochan_stark::{
    air::{BaseAirBuilder, PublicValues, MONEROCHANAirBuilder, MONEROCHAN_PROOF_NUM_PV_ELTS},
    Word,
};

use crate::{
    air::{MemoryAirBuilder, MONEROCHANCoreAirBuilder},
    cpu::{
...
where
    AB: MONEROCHANCoreAirBuilder + AirBuilderWithPublicValues,
    AB::Var: Sized,
{
    #[inline(never)]
...
impl CpuChip {
    ...
    pub(crate) fn eval_shard_clk<AB: MONEROCHANAirBuilder>(
        &self,
        builder: &mut AB,
    ...
    /// for non-real rows.
    pub(crate) fn eval_pc<AB: MONEROCHANAirBuilder>(
        &self,
        builder: &mut AB,
        local: &CpuCols<AB::Var>,
        next: &CpuCols<AB::Var>,
    ...
    pub(crate) fn eval_is_real<AB: MONEROCHANAirBuilder>(
        &self,
        builder: &mut AB,
        local: &CpuCols<AB::Var>,
        next: &CpuCols<AB::Var>,
