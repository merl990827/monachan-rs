...
use monerochan_core_executor::events::{ByteRecord, FieldOperation};
use monerochan_derive::AlignedBorrow;
use monerochan_stark::air::{Polynomial, MONEROCHANAirBuilder};

use super::{
    util::{compute_root_quotient_and_shift, split_u16_limbs_to_u8_limbs},
    util_air::eval_field_operation,
};
use monerochan_curves::params::{FieldParameters, Limbs};
            ...
            FieldOperation::Div => {
                ...
                cfg_if::cfg_if! {
                    if #[cfg(feature = "bigint-rug")] {
                        use monerochan_curves::utils::{biguint_to_rug, rug_to_biguint};
...
impl<V: Copy, P: FieldParameters> FieldOpCols<V, P> {
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn eval_variable<AB: MONEROCHANAirBuilder<Var = V>>(
        &self,
        builder: &mut AB,
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn eval_mul_and_carry<AB: MONEROCHANAirBuilder<Var = V>>(
        &self,
        builder: &mut AB,
        a: &(impl Into<Polynomial<AB::Expr>> + Clone),
        b: &(impl Into<Polynomial<AB::Expr>> + Clone),
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn eval_with_modulus<AB: MONEROCHANAirBuilder<Var = V>>(
        &self,
        builder: &mut AB,
        a: &(impl Into<Polynomial<AB::Expr>> + Clone),
        b: &(impl Into<Polynomial<AB::Expr>> + Clone),
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn eval_with_polynomials<AB: MONEROCHANAirBuilder<Var = V>>(
        &self,
        builder: &mut AB,
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn eval<AB: MONEROCHANAirBuilder<Var = V>>(
        &self,
        builder: &mut AB,
        a: &(impl Into<Polynomial<AB::Expr>> + Clone),
        b: &(impl Into<Polynomial<AB::Expr>> + Clone),
...
mod tests {
    ...
    use monerochan_core_executor::{ExecutionRecord, Program};
    use monerochan_curves::params::FieldParameters;
    use monerochan_stark::{
        air::{MachineAir, MONEROCHANAirBuilder},
        StarkGenericConfig,
    };
    ...
    use rand::thread_rng;
    use monerochan_core_executor::events::ByteRecord;
    use monerochan_curves::{
        edwards::ed25519::Ed25519BaseField, weierstrass::secp256k1::Secp256k1BaseField,
    };
    use monerochan_derive::AlignedBorrow;
    use monerochan_stark::baby_bear_poseidon2::BabyBearPoseidon2;
    use std::mem::size_of;
