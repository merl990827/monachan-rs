...
pub use monerochan_primitives::consts::fd::*;
...
mod ecrecover {
    use monerochan_curves::{k256, p256};
...
pub fn hook_ed_decompress(_: HookEnv, buf: &[u8]) -> Vec<Vec<u8>> {
    ...
    let modulus = monerochan_curves::edwards::ed25519::Ed25519BaseField::modulus();
...
mod bls {
    ...
    use monerochan_curves::{params::FieldParameters, weierstrass::bls12_381::Bls12381BaseField, Zero};
...
pub(crate) mod deprecated_hooks {
    ...
    #[must_use]
    pub fn hook_ed_decompress(_: HookEnv, buf: &[u8]) -> Vec<Vec<u8>> {
        let Ok(point) = monerochan_curves::curve25519_dalek::CompressedEdwardsY::from_slice(buf) else {
            return vec![vec![0]];
        };

        if monerochan_curves::edwards::ed25519::decompress(&point).is_some() {
            vec![vec![1]]
        } else {
            vec![vec![0]]
        }
