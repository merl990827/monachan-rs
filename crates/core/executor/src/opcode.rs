//! Opcodes for the MONEROCHAN zkVM.
...
///
/// While the MONEROCHAN zkVM targets the RISC-V ISA, it uses a custom instruction encoding that uses
/// a different set of opcodes. The main difference is that the MONEROCHAN zkVM encodes register
/// operations and immediate operations as the same opcode. For example, the RISC-V opcodes ADD and
/// ADDI both become ADD inside the MONEROCHAN zkVM. We utilize flags inside the instruction itself to
