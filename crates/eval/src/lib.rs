...
use monerochan_prover::{components::MONEROCHANProverComponents, utils::get_cycles, MONEROCHANProver};
use monerochan::{MONEROCHANContext, MONEROCHANStdin};
...
#[derive(Parser, Clone)]
#[command(about = "Evaluate the performance of MONEROCHAN on programs.")]
struct EvalArgs {
...
pub async fn evaluate_performance<C: MONEROCHANProverComponents>(
...
fn run_evaluation<C: MONEROCHANProverComponents>(
    program_name: &str,
    elf: &[u8],
    ...
    let prover = MONEROCHANProver::<C>::new();
    let (_, pk_d, program, vk) = prover.setup(elf);

    let context = MONEROCHANContext::default();
...
fn format_results(args: &EvalArgs, results: &[PerformanceReport]) -> Vec<String> {
    ...
    vec!["*MONEROCHAN Performance Test Results*\n".to_string(), detail_text, table_text]
}
...
async fn post_to_github_pr(
    ...
    let comments_response = client
        ...
        .header("User-Agent", "monerochan-perf-bot")
        .send()
        .await?;
...
mod tests {
    ...
    fn test_format_results() {
        ...
        let args = EvalArgs {
            ...
            repo_owner: Some("monero-chan-foundation".to_string()),
            repo_name: Some("monerochan".to_string()),
        ...
        assert!(formatted_results[0].contains("MONEROCHAN Performance Test Results"));
