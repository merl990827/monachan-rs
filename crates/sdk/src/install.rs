...
pub fn try_install_circuit_artifacts(artifacts_type: &str) -> PathBuf {
    ...
    if build_dir.exists() {
        eprintln!(
            "[monerochan] {} circuit artifacts already seem to exist at {}. if you want to re-download them, delete the directory",
            artifacts_type,
            build_dir.display()
        );
    } else {
        cfg_if! {
            if #[cfg(any(feature = "network", feature = "network"))] {
                eprintln!(
                    "[monerochan] {} circuit artifacts for version {} do not exist at {}. downloading...",
                    artifacts_type,
...
pub fn install_circuit_artifacts(build_dir: PathBuf, artifacts_type: &str) {
    ...
    eprintln!("[monerochan] downloaded {} to {:?}", download_url, build_dir.to_str().unwrap(),);
}
