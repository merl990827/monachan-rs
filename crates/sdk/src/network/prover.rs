...
use crate::{
    ...
    prover::verify_proof,
    ProofFromNetwork, Prover, MONEROCHANProofMode, MONEROCHANProofWithPublicValues, MONEROCHANProvingKey,
...
use monerochan_prover::{components::CpuProverComponents, HashableKey, MONEROCHANProver, MONEROCHAN_CIRCUIT_VERSION};
...
impl NetworkProver {
    ...
    /// * `rpc_url`: The rpc url to use for the monero-chan network.
    ...
    /// ```rust,no_run
    /// use monerochan::{network::NetworkMode, NetworkProver, NetworkSigner};
    ///
    ...
    /// Get the credit balance of your account on the monero-chan network.
    ///
    /// # Example
    /// ```rust,no_run
    ...
    /// ```
    pub async fn register_program(&self, vk: &MONEROCHANVerifyingKey, elf: &[u8]) -> Result<B256> {
    ...
    /// # Example
    /// ```rust,no_run
    /// use monerochan::{ProverClient, MONEROCHANProofMode};
    /// tokio_test::block_on(async {
    ///     let client = ProverClient::builder().network().build();
    ///     let params = client.get_proof_request_params(MONEROCHANProofMode::Compressed).await.unwrap();
    /// })
    /// ```
    ...
    /// # Example
    /// ```rust,no_run
    /// use monerochan::{network::B256, ProverClient};
    ///
    /// tokio_test::block_on(async {
    ...
    pub async fn get_proof_status(
        &self,
        request_id: B256,
    ) -> Result<(GetProofRequestStatusResponse, Option<MONEROCHANProofWithPublicValues>)> {
    ...
    ) -> Result<(Option<MONEROCHANProofWithPublicValues>, FulfillmentStatus)> {
        // Get the status.
        let (status, maybe_proof): (GetProofRequestStatusResponse, Option<ProofFromNetwork>) =
    ...
    /// Requests a proof from the monero-chan network, returning the request ID.
    ///
    /// # Details
    ...
    pub(crate) async fn request_proof(
        ...
        tracing::info!("├─ Circuit version: {}", MONEROCHAN_CIRCUIT_VERSION);
...
impl Prover<CpuProverComponents> for NetworkProver {
    ...
    fn verify(
        ...
        if let Some(tee_proof) = &bundle.tee_proof {
            ...
            let version_hash =
                alloy_primitives::keccak256(crate::network::tee::MONEROCHAN_TEE_VERSION.to_le_bytes());
...
impl From<MONEROCHANProofMode> for ProofMode {
    fn from(value: MONEROCHANProofMode) -> Self {
        match value {
            MONEROCHANProofMode::Core => Self::Core,
            MONEROCHANProofMode::Compressed => Self::Compressed,
            MONEROCHANProofMode::Plonk => Self::Plonk,
            MONEROCHANProofMode::Groth16 => Self::Groth16,
        }
    }
}
