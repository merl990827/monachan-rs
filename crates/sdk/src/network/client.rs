...
use monerochan_prover::{HashableKey, MONEROCHANVerifyingKey};
...
impl NetworkClient {
    ...
    pub fn get_vk_hash(vk: &MONEROCHANVerifyingKey) -> Result<B256> {
    ...
    /// * `version`: The version of the MONEROCHAN circuits to use.
                ...
                self.with_retry(
                    || async {
                        ...
                        let request_body = AuctionRequestProofRequestBody {
                            nonce,
                            version: format!("monerochan-{version}"),
                            vk_hash: vk_hash.to_vec(),
                            mode: mode.into(),
