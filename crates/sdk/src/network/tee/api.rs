...
impl TEEResponse {
    ...
    pub fn as_prefix_bytes(&self) -> Vec<u8> {
        ...
        // Include the version of the MONEROCHAN circuit.
        let version_bytes = super::MONEROCHAN_TEE_VERSION.to_le_bytes();
    ...
    /// The selector for the TEE verifier.
    fn selector() -> [u8; 4] {
        alloy_primitives::keccak256("MONEROCHANTeeVerifier")[0..4].try_into().unwrap()
    }
}
