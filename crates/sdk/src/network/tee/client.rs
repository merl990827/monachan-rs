...
use super::MONEROCHAN_TEE_VERSION;
...
impl Client {
    ...
    pub async fn execute(&self, request: TEERequest) -> Result<TEEResponse, ClientError> {
        ...
        let payload: EventPayload = self
            ...
            .header("X-MONEROCHAN-Tee-Version", MONEROCHAN_TEE_VERSION)
    ...
    /// Get the list of signers for the TEE server corresponding to the current MONEROCHAN circuit version.
    ///
    /// This function will send a request to the TEE server, and await a response.
    ///
    /// # Errors
