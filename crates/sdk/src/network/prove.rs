...
use crate::{
    ...
    NetworkProver, MONEROCHANProofMode, MONEROCHANProofWithPublicValues,
};
...
impl NetworkProveBuilder<'_> {
    ...
    /// # Example
    /// ```rust,no_run
    /// use monerochan::{network::FulfillmentStrategy, Prover, ProverClient, MONEROCHANStdin};
    ///
    /// let elf = &[1, 2, 3];
    /// let stdin = MONEROCHANStdin::new();
    ///
    ...
    /// not [`MONEROCHANProofMode::Compressed`], as this mode will be verified within the `VApp`.
    ///
    /// # Example
    /// ```rust,no_run
    /// use alloy_primitives::Address;
    ...
    ///
    /// # Details
    /// This method will request a proof from the monero-chan network. If the prover fails to request
    ...
    /// Request a proof from the monero-chan network asynchronously.
    ///
    /// # Details
    /// This method will request a proof from the monero-chan network asynchronously. If the prover fails
    ...
    /// ```
    pub async fn run_async(mut self) -> Result<MONEROCHANProofWithPublicValues> {
...
impl<'a> IntoFuture for NetworkProveBuilder<'a> {
    type Output = Result<MONEROCHANProofWithPublicValues>;
