...
#[derive(Clone)]
pub struct MONEROCHANContext<'a> {
    /// The registry of hooks invocable from inside MONEROCHAN.
    ///
...
pub struct MONEROCHANContextBuilder<'a> {
...
impl<'a> MONEROCHANContext<'a> {
    /// Create a new context builder. See [`MONEROCHANContextBuilder`] for more details.
    #[must_use]
    pub fn builder() -> MONEROCHANContextBuilder<'a> {
        MONEROCHANContextBuilder::new()
    }
}

impl<'a> MONEROCHANContextBuilder<'a> {
    /// Create a new [`MONEROCHANContextBuilder`].
    ///
    /// Prefer using [`MONEROCHANContext::builder`].
    #[must_use]
    pub fn new() -> Self {
        MONEROCHANContextBuilder::default()
    }

    /// Build and return the [`MONEROCHANContext`].
    ///
    /// Clears and resets the builder, allowing it to be reused.
    pub fn build(&mut self) -> MONEROCHANContext<'a> {
    ...
    ///
    /// The verifier is used to sanity check `verify_monerochan_proof` during runtime.
...
/// The IO options for the [`MONEROCHANExecutor`].
///
/// This struct is used to redirect the `stdout` and `stderr` of the [`MONEROCHANExecutor`].
#[derive(Default)]
...
#[cfg(test)]
mod tests {
    use crate::{subproof::NoOpSubproofVerifier, MONEROCHANContext};

    #[test]
    fn defaults() {
        let MONEROCHANContext { hook_registry, subproof_verifier, max_cycles: cycle_limit, .. } =
            MONEROCHANContext::builder().build();
    ...
    fn without_default_hooks() {
        let MONEROCHANContext { hook_registry, .. } =
            MONEROCHANContext::builder().without_default_hooks().build();
    ...
    fn with_custom_hook() {
        let MONEROCHANContext { hook_registry, .. } =
            MONEROCHANContext::builder().hook(30, |_, _| vec![]).build();
    ...
    fn without_default_hooks_with_custom_hook() {
        let MONEROCHANContext { hook_registry, .. } =
            MONEROCHANContext::builder().without_default_hooks().hook(30, |_, _| vec![]).build();
    ...
    fn subproof_verifier() {
        ...
        let MONEROCHANContext { subproof_verifier, .. } =
            MONEROCHANContext::builder().subproof_verifier(&verifier).build();
