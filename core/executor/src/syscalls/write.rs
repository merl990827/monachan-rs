...
impl Syscall for WriteSyscall {
    ...
    fn execute(
        ...
        if fd == 1 {
            ...
            if std::env::var("MONEROCHAN_ALLOW_DEPRECATED_HOOKS")
                .map(|r| {
                    r.parse::<bool>().expect("failed to parse MONEROCHAN_ALLOW_DEPRECATED_HOOKS as bool")
                })
                .unwrap_or(false)
            {
                ...
                panic!(
                    "You are using reserved file descriptor {fd} that is not supported on MONEROCHAN versions >= v4.0.0. \
