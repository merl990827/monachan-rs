...
fn main() {
    ...
    //
    // let mut stdout = MONEROCHANProver::execute(REGEX_IO_ELF, stdin).expect("proving failed");
    ...
    let deserialized_proof =
        MONEROCHANProofWithPublicValues::load("proof-with-pis").expect("loading proof failed");
