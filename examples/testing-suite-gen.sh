...
for program in "${PROGRAMS[@]}"; do
    ...
    if [ -d "$script_dir" ]; then
        ...
        MONEROCHAN_DUMP=1 cargo run --release -- --prove
        aws s3 cp stdin.bin "s3://monerochan-testing-suite/v4/$program_name/stdin.bin"
        aws s3 cp program.bin "s3://monerochan-testing-suite/v4/$program_name/program.bin"
