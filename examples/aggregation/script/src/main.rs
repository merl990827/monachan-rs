//! A simple example showing how to aggregate proofs of multiple programs with MONEROCHAN.

use monerochan::{
    include_elf, HashableKey, ProverClient, MONEROCHANProof, MONEROCHANProofWithPublicValues, MONEROCHANStdin,
...
struct AggregationInput {
    pub proof: MONEROCHANProofWithPublicValues,
...
fn main() {
    ...
    tracing::info_span!("aggregate the proofs").in_scope(|| {
        ...
        // witnessed by the prover during the recursive aggregation process inside MONEROCHAN itself.
        for input in inputs {
            let MONEROCHANProof::Compressed(proof) = input.proof.proof else { panic!() };
