...
fn main() {
    ...
    let (_, _) = client.execute(NORMAL_ELF, &MONEROCHANStdin::new()).run().expect("proving failed");

    // Execute the report program.
    let (_, report) = client.execute(REPORT_ELF, &MONEROCHANStdin::new()).run().expect("proving failed");
